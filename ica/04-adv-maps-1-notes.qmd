...
mn_cities <- st_read("~/Documents/GitHub/ds212-portfolio-ebyanh/ica/Data Folder/City_And_Township_Population_Centers/City_And_Township_Population_Centers.shp")
mn_water <- st_read("~/Documents/GitHub/ds212-portfolio-ebyanh/ica/Data Folder/Lakes_and_Rivers/Lakes_and_Rivers.shp")
```
...
ggplot(mn_cities)+ geom_sf()
ggplot(mn_water) + geom_sf()
...
transform_and_plot(mn_cities, new_crs = "EPSG:26992")
transform_and_plot(mn_water, new_crs = "EPSG:2810")
...
I would say that it would not be exactly accurate but it would be close enough
...
ggplot(mn_cities)+ geom_sf()+ ggthemes::theme_map()
ggplot(mn_water)+ geom_sf()+ ggthemes::theme_map()
ggplot(mn_counties)+ geom_sf()+ ggthemes::theme_map()
...
```{r mn_map_add_color}
ggplot() +
    geom_sf(data = mn_counties, fill = "lightgrey") + 
...
It gives you an easy to look like color pallate so you can organize your data better
- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?
It gives you diffirent options of viridis color palates for you.
...
Larger cities are loctaed in places that have lower elevation, and this can connect because places with higher elevation have less people since there are less resources there.
...
```{r}
ggplot() +
    geom_sf(data = mn_counties, fill = "lightgrey") + geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) +
  geom_sf(data = mn_cities |> filter(population >= 10000), mapping = aes(color = population, size = population)) + # cities layer
    scale_color_viridis_c() + # continuous (gradient) color scale
    labs(title = "Minnesota Cities with Population >= 10,000") + 
    ggthemes::theme_map() +
    theme(legend.position = "bottom") # move legend +
    scale_fill_gradient('darkgreen','white') 
...
seven_countyarea <- mn_counties |>
    ...
    st_bbox()
seven_countyarea <- st_as_sfc(seven_countyarea)

elevation <- elevatr::get_elev_raster(mn_counties |> st_crop(seven_countyarea), z = 9, clip = "bbox")
elevation_terra <- terra::rast(elevation)
terra::crs(elevation_terra) <- terra::crs(mn_counties)
