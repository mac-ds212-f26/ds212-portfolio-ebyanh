...
- What specific variables (from the data codebook) underlie the visualization? month, day, CulmPrec, RecordLow`/`RecordHigh\`, Record, RecodeText, PrecipYr, RecordPrecip
- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs? The grey part of the chart shows the expected average temperature during the year while red part will show the actual average temperature of that day. The graph uses 365 red rectangles stacked next to each other to show the actual average high/low temperature for that specific day. The cumulative monthly precipitaion is also showed with another line graph using a blue outline with grey shading under the line.
...
weather|>
  ggplot()+ 
  geom_linerange(aes(x = dateInYear, ymin= RecordLow , ymax = RecordHigh,colour = "grey"))
```
